...
ReliableServer.prototype.onMessage = function (socket, data) {
  ...
  var session = this.sessions[socket._sid],
      self = this;

  data.forEach(function(packet) {
    var id = packet[0];
    self.lastSeen = id;
    self.seenObj[id] = true;

    var packetObj = {type: 'ack', data: id};
    socket._send(parser.encodePacket(packeObj));
  });
};
      ...
      if (packet.type === 'error') {
        ...
        if (typeof oldHandler === 'function') {
          if (packet.type === 'message') {
            var packetID = packet.data[0];
            // but only if we haven't seen it before
            if (!self.seenObj(packetID)) {
