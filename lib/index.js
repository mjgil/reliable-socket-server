...
ReliableServer.prototype.onOpen = function (socket) {
  ...
  var self = this,
      ...
      tempSession = this.temp_sessions[socket._tid],
      sendPacket;
  ...
  thisSession.seenObj = tempSession.seenObj;
  thisSession.lastSeen = tempSession.lastSeen;

  this.flushTempSession(socket);
};
 ...
 * @api private
 */
ReliableServer.prototype.onAck = function (socket, id) {
  debug('onAck called');
 ...
 * @param  {Object} socket
 * @param  {Array} data [[<id>,<data>],[<id>,<data>]]
 * @param  {Function} oldHandler old .on('data') handler
 * @api private
 */
ReliableServer.prototype.onMessage = function (socket, data, oldHandler) {
  debug('onMessage called');
  var session = this.getSession(socket),
      self = this;

  if (!session) {
    var tempSession = this.temp_sessions[socket._tid];
    data.forEach(function(packet) {
      tempSession.writeBuffer.push(packet[1]);
    });
  }
  if (session) {
    data.forEach(function(packet) {
  ...
  function buildReplacementDataHandler(socket, oldHandler) {
    var newFN = function(data) {
      ...
      if (packet.type === 'error') {
        ...
        self[fnName](socket, packet.data, oldHandler);
      }
...
ReliableServer.prototype.getSession = function(socket) {
  ...
  if (!didHanshake) {
    ...
    return this.sessions[socket._sid];
  }
};
...
/**
 * Flushes the temporary session associated with a socket
 * 
 * @param  {Object} socket 
 * @api private
 */
ReliableServer.prototype.flushTempSession = function(socket) {
  debug('flushTempSession called');
  var self = this,
  bufferedPackets = [],
  session = this.temp_sessions[socket._tid];

  session.writeBuffer.forEach(function(data) {
    var packet = self.buildPacket(data, socket._sid);
    var packetID = packet[0];
    session.seenObj[packetID] = true;
    session.lastSeen = packetID;
